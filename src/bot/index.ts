...
🔗 ${paper.link}
    ...
    if (data.startsWith("export:")) {
      const format = data.replace("export:", "");
      await context.answer();
      ...
      let content: string;
      let filename: string;
      const exportFormat = format as "bibtex" | "csv";
...
⏱️ Link expires in 10 minutes.`,
        ...
        // Fallback: send as inline code block if PUBLIC_URL not configured
        // Truncate if too long for Telegram message (4096 chars max)
        ...
        try {
          await context.message?.editText(
            `📥 ${exportFormat === "bibtex" ? "BibTeX" : "CSV"} Export:\n\n\`\`\`\n${displayContent}\n\`\`\`${truncated ? "\n\n⚠️ Output truncated. Set PUBLIC_URL environment variable to enable file downloads." : ""}`,
            { parse_mode: "Markdown" },
          );
        } catch {
          await context.message?.send(
